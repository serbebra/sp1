//! The `internal_linear` operation for the Poseidon2 permutation.
//!
...
pub(crate) fn internal_linear_permute_mut<T, const WIDTH: usize>(input: &mut [T; WIDTH])
where
...
/// A set of columns needed to compute the `internal_linear` of the input state.
#[derive(AlignedBorrow, Default, Debug, Clone, Copy)]
#[repr(C)]
pub struct InternalLinearPermuteOperation<T> {
    pub result: [T; P2_WIDTH],
}

impl<F: Field> InternalLinearPermuteOperation<F> {
    pub fn populate(&mut self, array: &[F; P2_WIDTH]) -> [F; P2_WIDTH] {
        self.result = *array;
        internal_linear_permute_mut(&mut self.result);
        self.result
    }
    ...
    pub fn eval<AB: CurtaAirBuilder>(
        ...
        cols: InternalLinearPermuteOperation<AB::Var>,
        is_real: AB::Var,
    ) {
        let result = {
            let mut input: [AB::Expr; P2_WIDTH] = input_state.map(|x| x.into());
            internal_linear_permute_mut::<AB::Expr, P2_WIDTH>(&mut input);
            input
        };
