...
use sp1_recursion_compiler::asm::{AsmConfig, VmBuilder};
...
fn test_compiler_nested_array_loop() {
    ...
    let mut array: Array<C, Array<C, Var<_>>> = builder.dyn_array(100);

    builder.range(0, array.len()).for_each(|i, builder| {
        let mut inner_array = builder.dyn_array::<Var<_>>(10);
